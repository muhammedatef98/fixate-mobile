...
export default function VerifyIdentityScreen() {
  ...
  const canSubmit = useMemo(() => (
    ...
    && !!frontUri
    && !submitting
  ), [fullName, docNumber, frontUri, submitting]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitUserVerification({
        ...
        backImageUri: backUri ?? undefined,
      });
      await refreshUser?.();
      Alert.alert(
      ...
      >
        ...
        >
          ...
          <Text style={styles.intro}>
            {isRTL
              ? 'لرفع علامة التوثيق على حسابك وعلى إعلاناتك في السوق، أرفق صورة هويتك الوطنية أو إقامتك. تبقى مستنداتك خاصة ولا تظهر للمستخدمين.'
              : 'To earn the verified mark on your profile and your market listings, upload your Saudi national ID or Iqama. Documents remain private and are never shown to other users.'}
          </Text>
          ...
          <Text style={styles.label}>
            {isRTL ? 'صورة الوجه الخلفي (اختياري)' : 'Document back photo (optional)'}
          </Text>
          <UploadTile
            uri={backUri}
            onPress={() => pickImage(setBackUri)}
            placeholder={isRTL ? 'اضغط لاختيار صورة الوجه الخلفي' : 'Tap to upload back side'}
            COLORS={COLORS}
            isRTL={isRTL}
          />
