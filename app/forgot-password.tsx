...
export default function ForgotPasswordScreen() {
  ...
  const verifyCode = async () => {
    ...
    if (code.length < 4 || code.length > 10) {
      Alert.alert(isRTL ? 'خطأ' : 'Error', isRTL ? 'الكود غير صحيح' : 'Code length is invalid');
      return;
    }
    setLoading(true);
    try {
  ...
  const setPassword = async () => {
    ...
    if (!check.isValid) {
      Alert.alert(
        isRTL ? 'خطأ' : 'Error',
        check.errors[0] || (isRTL ? 'كلمة المرور ضعيفة' : 'Weak password')
      );
      return;
    }
    setLoading(true);
    try {
      ...
      const target = isAdmin ? '/admin' : isTechnician ? '/(technician)' : '/(customer)';
      Alert.alert(
        isRTL ? 'تم' : 'Done',
        isRTL
          ? 'تم تحديث كلمة المرور. يمكنك الدخول الآن.'
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity
          onPress={() =>
            step === 'email' ? router.back() : setStep(step === 'newPassword' ? 'otp' : 'email')
          }
            ...
            {step === 'email' && (
              <>
                ...
                <Text style={styles.sub}>
                  {isRTL
                    ? 'سنرسل كود مكوّن من 6 أرقام لإعادة تعيين كلمة المرور'
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.btn,
                    {
                      ...
                      opacity: !validateEmail(email.trim()) || loading ? 0.5 : 1,
                    },
                  ]}
                  accessibilityRole="button"
            ...
            {step === 'otp' && (
              <>
                ...
                <TextInput
                  ...
                  style={[
                    styles.otpInput,
                    { color: COLORS.text, borderColor: COLORS.border, writingDirection: 'ltr' },
                  ]}
                  textAlign="center"
                  autoFocus
                  maxLength={10}
                ...
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    {
                      backgroundColor: COLORS.primary,
                      opacity: code.length < 4 || loading ? 0.5 : 1,
                    },
                  ]}
                  accessibilityRole="button"
              ...
              <>
                ...
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    {
                      backgroundColor: COLORS.primary,
                      opacity: loading || !validatePassword(newPassword).isValid ? 0.5 : 1,
                    },
                  ]}
                  accessibilityRole="button"
